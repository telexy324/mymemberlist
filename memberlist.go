...
type Memberlist struct {
	sequenceNum uint32 // Local sequence number 接受indrectping时会加1
	incarnation uint32 // Local incarnation number (atomic的)
	...
	nodeMap    map[string]*nodeState // Maps Addr.String() -> NodeState 存储本节点知道的其他节点的状态
	nodeTimers map[string]*suspicion // Maps Addr.String() -> suspicion timer 一个定时器，判断节点是否已死
	awareness  *awareness
...
func newMemberlist(conf *Config) (*Memberlist, error) {
	...
	if transport == nil {
		...
		makeNetRetry := func(limit int) (*NetTransport, error) { //给予一定的重试次数，避免系统故障（在不指定port绑定port时）
			var err error
	...
	m := &Memberlist{
		...
		highPriorityMsgQueue: list.New(), //创建两个双向链表
		lowPriorityMsgQueue:  list.New(),
		nodeMap:              make(map[string]*nodeState), //存储节点的状态
		nodeTimers:           make(map[string]*suspicion), //存储节点的怀疑状态（定时器）
...
func (m *Memberlist) Leave(timeout time.Duration) error {
	...
	if !m.hasLeft() { //leave标志位不是1，表示没离开
		atomic.StoreInt32(&m.leave, 1) //置为1
