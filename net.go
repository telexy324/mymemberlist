...
type indirectPingReq struct {
	SeqNo  uint32
	Target []byte //目标的ip
	Port   uint16 //目标节点的port
	Node   string //真正的目标节点
	Nack   bool // true if we'd like a nack back //就是帮忙节点收到了请求，但发送ping失败了
}
...
// that the indirect ping attempt happened but didn't succeed. indirectping发出了，在规定时间内未收到回复，则告知原节点（请求节点）indrectping失败
...
// transferring out node states pushpull中代表节点状态的结构体
...
func (m *Memberlist) handleConn(conn net.Conn) {
	...
	conn.SetDeadline(time.Now().Add(m.config.TCPTimeout)) //加上超时设置，超时后所有的请求会得到timeout错误，刷新后可以继续使用
	msgType, bufConn, dec, err := m.readStream(conn) //处理加密，压缩消息等，返回消息类型，消息等（真的消息类型，不含加密、压缩头）
	if err != nil {
		if err != io.EOF {
			m.logger.Printf("[ERR] memberlist: failed to receive: %s %s", err, LogConn(conn))

			resp := errResp{err.Error()}
			out, err := encode(errMsg, &resp) //出错，则把错误编码返回
			if err != nil {
	...
	switch msgType {
	case userMsg: //用户自定义的信息，调用用户注册的delegate中的notifyMsg
	...
	case pushPullMsg:
		...
		defer atomic.AddUint32(&m.pushPullReq, ^uint32(0)) //相当于减1，如果为0时，执行此就是加到最大值
		...
		join, remoteNodes, userState, err := m.readRemoteState(bufConn, dec) //是否join,发过来的节点状态，用户自定义消息
		if err != nil {
...
func (m *Memberlist) ingestPacket(buf []byte, from net.Addr, timestamp time.Time) {
	...
	if m.config.EncryptionEnabled() {
		// Decrypt the payload
		plain, err := decryptPayload(m.config.Keyring.GetKeys(), buf, nil) //udp消息没有消息头（data,消息类型，长度）
		if err != nil {
			if !m.config.GossipVerifyIncoming { //不要求强制加密
...
func (m *Memberlist) handleCommand(buf []byte, from net.Addr, timestamp time.Time) {
	...
	case deadMsg:
		fallthrough
	case userMsg: //把上三种情况都放到这儿判断
		// Determine the message queue, prioritize alive
		queue := m.lowPriorityMsgQueue //suspect,dead,user都进这个
		if msgType == aliveMsg { //alive用这个
		...
		if queue.Len() >= m.config.HandoffQueueDepth {
			...
			queue.PushBack(msgHandoff{msgType, buf, from}) //插到队尾
		}
		m.msgQueueLock.Unlock()

		// Notify of pending message
		select {
		case m.handoffCh <- struct{}{}: //往handoffch中发一个值，这个ch的容量为1
		default:
		}
...
// getNextMessage returns the next message to process in priority order, using LIFO 类似于栈
func (m *Memberlist) getNextMessage() (msgHandoff, bool) {
	m.msgQueueLock.Lock()
	defer m.msgQueueLock.Unlock()

	if el := m.highPriorityMsgQueue.Back(); el != nil { //先从高权限队列中找
...
// blocking the listener which may cause ping/ack messages to be delayed. 独立出来防止阻塞ping/ack信息，就是处理alive,suspect,dead,usermsg
func (m *Memberlist) packetHandler() {
	for {
		select {
		case <-m.handoffCh:
			for {
				msg, ok := m.getNextMessage() //得到处理队列里的消息
				if !ok {
					break
				}
				msgType := msg.msgType
...
func (m *Memberlist) handlePing(buf []byte, from net.Addr) {
	...
	if p.Node != "" && p.Node != m.config.Name { //必须发给自己
...
func (m *Memberlist) handleIndirectPing(buf []byte, from net.Addr) {
	...
	ping := ping{SeqNo: localSeqNo, Node: ind.Node} //向目标节点发ping

	// Setup a response handler to relay the ack
	cancelCh := make(chan struct{})
	respHandler := func(payload []byte, timestamp time.Time) { //构建一个回调函数，返回给发送请求者
	...
	// Setup a timer to fire off a nack if no ack is seen in time.如果需要nack,就起select，因为回调函数会关掉cancelCh,所以cancelch先到，表示成功得到了回复，不再需要nack了，计时器先到表示未得到回复，则发送nack
	if ind.Nack {
		go func() {
			select {
			case <-cancelCh:
...
func (m *Memberlist) sendMsg(addr string, msg []byte) error { //用于批量的发送udp消息
...
func (m *Memberlist) rawSendMsgPacket(addr string, node *Node, msg []byte) error {
	...
	if node == nil { //如果node是空的，则拆分传进来的addr,用IP地址（域名）当作node名字，在节点的node表里查找，如果有就使用这个node
	...
	// ProtocolVersion >= 5 目标节点的pmax大于5则加上crc校验
	...
	_, err := m.transport.WriteTo(msg, addr) //udp的消息没有go程，直接用
	return err
}
...
func (m *Memberlist) sendLocalState(conn net.Conn, join bool) error {
	...
	m.nodeLock.RLock()
	localNodes := make([]pushNodeState, len(m.nodes)) //准备本地所有已知节点的信息，转化为pushodestate的形式
	...
	if m.config.Delegate != nil {
		userData = m.config.Delegate.LocalState(join) //表示把本地额外的用户自定义的消息转化为[]byte
	}
	...
	// Send our node state
	header := pushPullHeader{Nodes: len(localNodes), UserStateLen: len(userData), Join: join} //组成消息头
	hd := codec.MsgpackHandle{}
	enc := codec.NewEncoder(bufConn, &hd)

	// Begin state push
	if _, err := bufConn.Write([]byte{byte(pushPullMsg)}); err != nil { //消息标志位
		return err
	}

	if err := enc.Encode(&header); err != nil { //编码消息头
		return err
	}
	for i := 0; i < header.Nodes; i++ {
		if err := enc.Encode(&localNodes[i]); err != nil { //编码要发送的本地知道的节点信息
			return err
		}
	}

	// Write the user state as well
	if userData != nil { //直接加上用户自定义信息
...
func (m *Memberlist) encryptLocalState(sendBuf []byte) ([]byte, error) {
	...
	buf.WriteByte(byte(encryptMsg)) //写入加密消息头

	// Write the size of the message
	sizeBuf := make([]byte, 4)
	encVsn := m.encryptionVersion()
	encLen := encryptedLength(encVsn, len(sendBuf)) //根据加密方式和原始长度算出需要加多少padding,算出加密后总长度
	binary.BigEndian.PutUint32(sizeBuf, uint32(encLen)) //大头方式写入加密消息的总长度
	buf.Write(sizeBuf)
...
func (m *Memberlist) decryptRemoteState(bufConn io.Reader) ([]byte, error) {
	...
	// an attack vector where a huge amount of state is sent 消息的1到5byte是长度？
	moreBytes := binary.BigEndian.Uint32(cipherText.Bytes()[1:5])
	if moreBytes > maxPushStateBytes { //最长20M？
...
func (m *Memberlist) readStream(conn net.Conn) (messageType, io.Reader, *codec.Decoder, error) {
	...
	if msgType == encryptMsg {
		...
		plain, err := m.decryptRemoteState(bufConn) //读取出加密的信息（纯明文）
		if err != nil {
			return 0, nil, nil, err
		}

		// Reset message type and bufConn
		msgType = messageType(plain[0]) //加密的信息需要从加密的信息里再读出msgtype（真正的msgtype）
		bufConn = bytes.NewReader(plain[1:]) //将余下的部分作为reader返回
	...
	hd := codec.MsgpackHandle{}
	dec := codec.NewDecoder(bufConn, &hd) //decoder用法，传入编好码的内容（bufconn）和编码器（hd），然后调用decode(v)方法，将内容解码到v中

	// Check if we have a compressed message
	if msgType == compressMsg {
		var c compress
		if err := dec.Decode(&c); err != nil { //解码到c中
			return 0, nil, nil, err
		}
		...
		msgType = messageType(decomp[0]) //先找消息类型

		// Create a new bufConn
		bufConn = bytes.NewReader(decomp[1:]) //然后消息内容
...
func (m *Memberlist) readRemoteState(bufConn io.Reader, dec *codec.Decoder) (bool, []pushNodeState, []byte, error) {
	...
	remoteNodes := make([]pushNodeState, header.Nodes) //nodes字段代表传过来多少个节点的状态

	// Try to decode all the states 解码所有传过来的状态
	for i := 0; i < header.Nodes; i++ {
	...
	return header.Join, remoteNodes, userBuf, nil //返回是否join,所有解码出的节点状态，除了pushpullheader，所有节点信息之外的未解码信息（用户自定义信息delegate）
}
...
func (m *Memberlist) mergeRemoteState(join bool, remoteNodes []pushNodeState, userBuf []byte) error {
	if err := m.verifyProtocol(remoteNodes); err != nil { //找到发过来节点和本地节点都满足的最大最小protocol,然后看这些节点是不是都满足,有一个不满足，就出错
		return err
	}
	...
	if join && m.config.Merge != nil {
		...
		if err := m.config.Merge.NotifyMerge(nodes); err != nil { //调用自己注册的mergedelegate方法
			return err
		}
	}
	...
	if userBuf != nil && m.config.Delegate != nil {
		m.config.Delegate.MergeRemoteState(userBuf, join) //接受远端传来的自定义信息
	}
	return nil
}
...
func (m *Memberlist) readUserMsg(bufConn io.Reader, dec *codec.Decoder) error {
	...
	if err := dec.Decode(&header); err != nil { //decode将值解码到传入的指针指向的值中
		return err
	}
