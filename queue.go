...
// (hence newer messages).有限传输队列，存储要发到集群的消息，消息传送次数有限，传送次数少的优先
type TransmitLimitedQueue struct {
	...
	NumNodes func() int //返回集群中节点的个数，每条消息传送多少次基于此，是动态的

	// RetransmitMult is the multiplier used to determine the maximum
	// number of retransmissions attempted.
	RetransmitMult int //在config中设定，算消息传送多少次时用到，与numnodes一起使用

	mu    sync.Mutex
	tq    *btree.BTree // stores *limitedBroadcast as btree.Item 一般的消息存在btree中
	tm    map[string]*limitedBroadcast //namequeue的消息存在map中
	idGen int64
}

type limitedBroadcast struct {
	transmits int   // btree-key[0]: Number of transmissions attempted. 传送次数
	msgLen    int64 // btree-key[1]: copied from len(b.Message()) 消息长度
	id        int64 // btree-key[2]: unique incrementing id stamped at submission time 唯一消息ID
	b         Broadcast //接口，invalidate,message(),finished()

	name string // set if Broadcast is a NamedBroadcast
}

// Less tests whether the current item is less than the given argument.
// 实现btree的item接口，需要实现比较Less方法
...
func (q *TransmitLimitedQueue) queueBroadcast(b Broadcast, initialTransmits int) {
	...
	lb := &limitedBroadcast{
		...
		msgLen:    int64(len(b.Message())), //encode后的消息，encode了msg和msgtype
		id:        id,
		b:         b, //memberlistbroadcast
	}
	unique := false
	...
	if lb.name != "" { //同名的直接finish
		...
		q.tq.Ascend(func(item btree.Item) bool {
			...
			case UniqueBroadcast:
				// noop
			default:
				if b.Invalidates(cur.b) { //同节点的信息直接finish
					cur.b.Finished()
...
func (q *TransmitLimitedQueue) getTransmitRange() (minTransmit, maxTransmit int) {
	...
	minItem, maxItem := q.tq.Min(), q.tq.Max() //按自己设定的less取回最大最小值
...
func (q *TransmitLimitedQueue) GetBroadcasts(overhead, limit int) [][]byte {
	...
	minTr, maxTr := q.getTransmitRange() //拿到最小的次数和最大的次数
	for transmits := minTr; transmits <= maxTr; /*do not advance automatically*/ {
		...
		q.tq.AscendRange(greaterOrEqual, lessThan, func(item btree.Item) bool {
			...
			if int64(len(cur.b.Message())) > free { //超出了范围
		...
		bytesUsed += overhead + len(msg) //udp的消息是组成compoundMsg发出去，包括消息长度和消息本身，消息长度的长度为2bytes
		...
		if keep.transmits+1 >= transmitLimit {
			...
			keep.transmits++
			reinsert = append(reinsert, keep) //发送之后也先不删除，总发送数加1，然后再重新插入到queue中
		}
	}
