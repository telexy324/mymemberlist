...
type nodeState struct {
	Node
	Incarnation uint32        // Last known incarnation number 世代信息
	State       nodeStateType // Current state 目前的状态3种
	StateChange time.Time     // Time last state change happened 上一次变更的时间
}
...
func (m *Memberlist) triggerFunc(stagger time.Duration, C <-chan time.Time, stop <-chan struct{}, f func()) {
	...
	select {
	case <-time.After(randStagger): //以随机的时间启动
	case <-stop:
		return
	}
	for {
...
// If this passes, it means that every node can understand each other. 检验别的节点发来的节点状态符不符合要求
func (m *Memberlist) verifyProtocol(remote []pushNodeState) error {
	...
	mindmax = math.MaxUint8 //maxpmin = 0 minpmax=255 maxdmin = 0 mindmax=255

	for _, rn := range remote {
		// If the node isn't alive, then skip it
		if rn.State != stateAlive { //只要活着的节点
			continue
		}
		...
		if rn.Vsn[0] > maxpmin { //允许的最低值（protocol version）
			maxpmin = rn.Vsn[0] // p最大的最小值
		}

		if rn.Vsn[1] < minpmax { //允许的最高值（protocol version） 2 为现在使用的version
			minpmax = rn.Vsn[1] // p最小的最大值
		}

		if rn.Vsn[3] > maxdmin { //允许的最低值（delegate version）
			maxdmin = rn.Vsn[3] // d最大的最小值
		}

		if rn.Vsn[4] < mindmax { //允许的最高值（delegate version）5 为现在使用的version
			mindmax = rn.Vsn[4] // d最小的最大值
		}
	}

	for _, n := range m.nodes {
		// Ignore non-alive nodes 忽略自己记录的非活节点
		if n.State != stateAlive {
			continue
		}

		if n.PMin > maxpmin { //将本地节点的protocol也统一
			maxpmin = n.PMin
		}
...
func (m *Memberlist) estNumNodes() int { //当作获取节点个数的函数
...
// nodeLock is held.
func (m *Memberlist) refute(me *nodeState, accusedInc uint32) { //参数是自己的状态和传过来的的自己的incarnation
	// Make sure the incarnation number beats the accusation.
	inc := m.nextIncarnation()
	if accusedInc >= inc {
		inc = m.skipIncarnation(accusedInc - inc + 1) //保证比传进来的大
	}
	me.Incarnation = inc
	...
	m.encodeAndBroadcast(me.Addr.String(), aliveMsg, a) //放到广播消息队列中（B树）
}
...
func (m *Memberlist) aliveNode(a *alive, notify chan struct{}, bootstrap bool) {
	...
	// ensures that we don't. 意思是自己离开的时候，需要一段过程，这时自己在别人那里还是alive的，所以别的节点可能发来自己alive的信息，这种要丢弃掉？
	if m.hasLeft() && a.Node == m.config.Name { //表示别的节点发过来的状态里含有自己，并且是alive的，但是实际自己已经离开了
		return
	}
	...
	if m.config.Alive != nil { //为每一个远端发过来的alive的node触发alivedelegate
		node := &Node{
			Name: a.Node,
			Addr: a.Addr,
			Port: a.Port,
	...
	if !ok { //本地没有这个节点，则以dead状态加入
		...
		// very high.
		n := len(m.nodes)
		offset := randomOffset(n) //得到一个根据n取模的值

		// Add at the end and swap with the node at the offset
		m.nodes = append(m.nodes, state)
		m.nodes[offset], m.nodes[n] = m.nodes[n], m.nodes[offset] //把这个节点插入到本地节点集的offset位置
	...
	if !bytes.Equal([]byte(state.Addr), a.Addr) || state.Port != a.Port { //传过来的节点本地有，但地址或端口不一致
	...
	isLocalNode := state.Name == m.config.Name //有记录，判断是不是本节点
	if a.Incarnation <= state.Incarnation && !isLocalNode { //不是本节点，并且incarnation比记录的小，丢弃
		return
	}
	...
	// Clear out any suspicion timer that may be in effect. 删除怀疑计时器
	delete(m.nodeTimers, a.Node)

	// Store the old state and meta data 存储原有状态和元信息
	oldState := state.State
	oldMeta := state.Meta

	// If this is us we need to refute, otherwise re-broadcast
	if !bootstrap && isLocalNode { //如果是本节点，且不是bootstrap，incarnation大于或等于
		...
		m.refute(state, a.Incarnation) //构建自己的新的incarnation信息，放入广播队列
		m.logger.Printf("[WARN] memberlist: Refuting an alive message")
	} else { //不是本节点，且incarnation大于
		m.encodeBroadcastNotify(a.Node, aliveMsg, a, notify) //放入广播队列

		// Update protocol versions if it arrived 更新本地的节点信息
		if len(a.Vsn) > 0 {
			state.PMin = a.Vsn[0]
			state.PMax = a.Vsn[1]
			state.PCur = a.Vsn[2]
...
func (m *Memberlist) suspectNode(s *suspect) {
	...
	if !ok { //以前没有记录的节点，有怀疑直接忽略
		return
	}

	// Ignore old incarnation numbers
	if s.Incarnation < state.Incarnation { //incarnation小于记录的也忽略
		return
	}
	...
	if timer, ok := m.nodeTimers[s.Node]; ok { //有这个节点的timer的话就去confirm一下
	...
	if state.State != stateAlive { //忽略不是alive的节点，如果不是alive的话是有问题的
		return
	}

	// If this is us we need to refute, otherwise re-broadcast
	if state.Name == m.config.Name { //是自己，refute
	...
	// multiplier.
	k := m.config.SuspicionMult - 2 //不知道有多少节点时设置初始的k

	// If there aren't enough nodes to give the expected confirmations, just
	// set k to 0 to say that we don't expect any. Note we subtract 2 from n
	// here to take out ourselves and the node being probed.
	n := m.estNumNodes() //获取现有的节点数
	if n-2 < k { //k设的太大，置为0
		k = 0
	}
	...
	fn := func(numConfirmations int) { //触发函数的参数是已经有多少确认数
		m.nodeLock.Lock()
		state, ok := m.nodeMap[s.Node]
		...
		if timeout {
			...
			d := dead{Incarnation: state.Incarnation, Node: state.Name, From: m.config.Name} //组成节点死亡信息
			m.deadNode(&d)
		}
	}
...
func (m *Memberlist) deadNode(d *dead) {
	...
	// Ignore old incarnation numbers
	if d.Incarnation < state.Incarnation { //小的都不处理
		return
	}

	// Clear out any suspicion timer that may be in effect.
	delete(m.nodeTimers, d.Node) //直接删掉timer
...
func (m *Memberlist) mergeState(remote []pushNodeState) {
	for _, r := range remote {
		...
		case stateDead:
			...
			fallthrough
		case stateSuspect: //别人发来的怀疑，我们也怀疑
