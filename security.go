...
// the PKCS7 algorithm. "Ignores" some bytes to compensate for IV 补足
func pkcs7encode(buf *bytes.Buffer, ignore, blockSize int) {
	n := buf.Len() - ignore
	more := blockSize - (n % blockSize)
	for i := 0; i < more; i++ {
		buf.WriteByte(byte(more)) //消息长度减去忽略的长度，不足block的倍数的话，差几，就在消息末尾补几个几
	}
}

// pkcs7decode is used to decode a buffer that has been padded 去掉补足
...
// for a message of given length 计算需要的buf长度
func encryptedLength(vsn encryptionVersion, inp int) int { //inp是传进来的原始长度
...
func encryptPayload(vsn encryptionVersion, key []byte, msg []byte, data []byte, dst *bytes.Buffer) error {
	...
	dst.WriteByte(byte(vsn)) //先把加密方式写入

	// Add a random nonce
	io.CopyN(dst, rand.Reader, nonceSize) //加入随机的nonce
	afterNonce := dst.Len()
...
func decryptPayload(keys [][]byte, msg []byte, data []byte) ([]byte, error) { //msg是tlv中的v,data是tlv中的tl表示没加密的部分
