...
type Config struct {
	...
	// at the expense of increased bandwidth. 广播消息重发送的指数，次数由这个指数和集群节点的数量共同决定
	RetransmitMult int
	...
	// still alive. 越大则suspect节点保存的时间越长
	SuspicionMult int
	...
	// usage. 设为0则停止push/pull，设的越小，交换的越频繁
	...
	ProbeInterval time.Duration //probe间隔
	ProbeTimeout  time.Duration //probe超时
	...
	// indirect UDP pings.
	DisableTcpPings bool //表示是否禁止tcp的ping就是在udp的ping失败后，同时发出udp的indirectping和再向那个节点发出tcp的ping中的那个tcp的ping

	// AwarenessMaxMultiplier will increase the probe interval if the node
	// becomes aware that it might be degraded and not meeting the soft real
	// time requirements to reliably probe other nodes. 节点意识到自己处于不健康状态时提升probe频率？
	...
	// to be gossiped that haven't been able to piggyback on probing messages. 不能够由probing消息携带的消息，就需要gossip
	// If this is set to zero, non-piggyback gossip is disabled. By lowering
	// this value (more frequent) gossip messages are propagated across
	// the cluster more quickly at the expense of increased bandwidth. gossip的间隔
	//
	// GossipNodes is the number of random nodes to send gossip messages to
	// per GossipInterval. Increasing this number causes the gossip messages
	// to propagate across the cluster more quickly at the expense of
	// increased bandwidth. gossip的数量
	//
	// GossipToTheDeadTime is the interval after which a node has died that
	// we will still try to gossip to it. This gives it a chance to refute.
	GossipInterval      time.Duration
	GossipNodes         int
	GossipToTheDeadTime time.Duration //标记为dead后保留的时间

	// GossipVerifyIncoming controls whether to enforce encryption for incoming
	// gossip. It is used for upshifting from unencrypted to encrypted gossip on
	// a running cluster. 进入消息是否强制加密
	GossipVerifyIncoming bool

	// GossipVerifyOutgoing controls whether to enforce encryption for outgoing
	// gossip. It is used for upshifting from unencrypted to encrypted gossip on
	// a running cluster. 发出消息是否强制加密
	...
	Events                  EventDelegate // 当集群内有节点加入、离开、元数据变更都会触发EventDelegate回调
	Conflict                ConflictDelegate // 当`Name`和`AdvertiseAddr`/`AdvertisePort`不对应时，会触发ConflictDelegate回调
	Merge                   MergeDelegate // 当某节点Join该集群时，集群内的每个节点会触发MergeDelegate的接口一次，用于接受新节点的数据。该接口可以是nil。
	Ping                    PingDelegate // 当探测其他节点时，会触发相关ping接口
	Alive                   AliveDelegate
